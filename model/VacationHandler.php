...
class VacationHandler {
    ...
    function remove() {
        ...
        $result = db_query("DELETE FROM $table_vacation WHERE email='$username'");
    ...
    function get_details() {
        ...
        if($result['rows'] == 1) {
            ...
            return array( 'subject' => $row['subject'],
                          'body' => $row['body'],
                          'active'  => $boolean ,
			  'datastartr' => $row['datainizio'],
			  'datastopr' => $row['datafine']);
        }
        return false;
    }
    /**
     ...
     */
    function set_away($subject, $body, $datainizio, $datafine) {
        ...
        if($result['rows'] == 1) {
            $result = db_query("UPDATE $table_vacation SET active = '$active', body = '$body', subject = '$subject', datainizio = '$datainizio', datafine = '$datafine' created = NOW() WHERE email = '$username'");
        }
        else {
            $tmp = preg_split ('/@/', $username);
            $domain = escape_string($tmp[1]);
            $result = db_query ("INSERT INTO $table_vacation (email,subject,body,domain,created,active,datainizio,datafine) VALUES ('$username','$subject','$body','$domain',NOW(),'$active','$datainizio','$datafine')");
        }
