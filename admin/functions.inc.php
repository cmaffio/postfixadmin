...
/**
 * Indica se l'utente e' abilitato per l'invio massivo
 * Returns false if neither (E.g. if not logged in)
 * @return String admin or user or (boolean) false.
 */
function authentication_get_mr() {
    if(isset($_SESSION['sessid'])) {
        if(isset($_SESSION['sessid']['mr'])) {
            return $_SESSION['sessid']['mr'];
        }
    }
    return false;
}
...
function authentication_has_role($role) {
    ...
    if(isset($_SESSION['sessid'])) {
        if(isset($_SESSION['sessid']['roles'])) {
            if(in_array($role, $_SESSION['sessid']['roles']) || in_array('global-admin', $_SESSION['sessid']['roles'])) {
                return true;
            }
        }
    }
...
function authentication_require_mr() {
    global $CONF;
    if(authentication_get_mr()) {
        return True;
    }
    header("Location: " . $CONF['postfix_admin_url'] );
}
...
function check_email_admin ($email) {
	...
	$result = db_query ("SELECT name FROM $table_mailbox WHERE username='$email' AND active='1'");
	if ($result['rows'] == 1) {
		return 1;
	}
	return 0;
...
//
function db_query ($query, $ignore_errors = 0, $link2 = NULL)
{
    global $CONF;
    global $DEBUG_TEXT;
    $result = "";
...
function create_admin($fUsername, $fPassword, $fPassword2, $fDomains, $no_generate_password=0)
{
    ...
    if (!check_email_admin ($fUsername))
    {
        $error = 1;
...
//	$password = pacrypt($fPassword);
        ...
        $result = db_query ("INSERT INTO " . table_by_key('admin') . " (username,created,modified) VALUES ('$fUsername',NOW(),NOW())");
        if ($result['rows'] != 1)
        {
...
function is_admin_role ($fUsername, $fPassword) {
	global $table_admin,$table_domain_admins;
...
function is_mr_role ($fUsername) {
	global $table_mailbox;

	$query = "SELECT mr FROM $table_mailbox WHERE username='$fUsername' AND mr > 0 AND active='1'";
	$result = db_query ($query);
	if ($result['rows'] == 1) {
...
function is_new ($fUsername) {
	global $table_mailbox;

	$result = db_query ("SELECT nuovo FROM $table_mailbox WHERE username='$fUsername' AND active='1' AND nuovo='1'");
	if ($result['rows'] == 1) {   
		$_SESSION['sessid']['nuovo'] = 'nuovo';
	} else {
		$_SESSION['sessid']['nuovo'] = 'attivo';
	}
}

function is_not_new ($fUsername) {
	global $table_mailbox;

	$result = db_query ("SELECT nuovo FROM $table_mailbox WHERE username='$fUsername' AND active='1' AND nuovo='1'");
	if ($result['rows'] == 1) {   
			$result = db_query ("UPDATE $table_mailbox SET nuovo='0' WHERE username='$fUsername' AND active='1' AND nuovo='1'");
			$_SESSION['sessid']['nuovo'] = 'attivo';
	}
}
