...
if (sizeof ($tMailbox) > 0) {
   ...
   if (authentication_get_usertype () == "global-admin") print "      <td>Spazio usato (MB)</td>\n";
   print "      <td>" . $PALANG['pOverview_mailbox_modified'] . "</td>\n";
   print "      <td>" . $PALANG['pOverview_mailbox_active'] . "</td>\n";
   if (authentication_get_usertype () == "global-admin") { print "      <td>Ultimo accesso</td>\n"; }
   $colspan = $colspan - 6;
   ...
   for ($i = 0; $i < sizeof ($tMailbox); $i++)
   {
      if ((is_array ($tMailbox) and sizeof ($tMailbox) > 0))
      {
         ...
         if (authentication_get_usertype () == "global-admin") {
            print "      <td>".$tMailbox[$i]['fsquota']."</td>\n";
         }
	 ...
	 if (authentication_get_usertype () == "global-admin") { 
		$ultimo = new DateTime($tMailbox[$i]['last_login']);
		$adesso = new DateTime("now");
		$interval = date_diff($ultimo, $adesso);
		...
		print "      <td>$bold1" . $tMailbox[$i]['last_login'] . "$bold2</td>\n"; 
	 }
