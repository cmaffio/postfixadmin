...
if ($_SERVER['REQUEST_METHOD'] == "GET")
{
   $vacancystart = "";
   $vacancystop = "";
   $result = db_query("SELECT * FROM $table_vacation WHERE email='$fUsername' AND datafine > NOW()");
   if ($result['rows'] == 1)
   {
      ...
      $tBody = $row['body'];
      $vacancystart = $row['datainizio'];
      $vacancystop = $row['datafine'];
   }
   ...
   if ((empty ($datastart))||(empty($datastop))){
   	...
   	if (!empty ($fBack) || !empty ($fChange))
   	{
      		...
      		if ($result['rows'] == 1)
      		{
         		...
         		// INIZIO MODIFICA
         		// $result = db_query ("UPDATE $table_vacation SET datainizio = '' WHERE email='$fUsername'");
         		// $result = db_query ("UPDATE $table_vacation SET datafine = '' WHERE email='$fUsername'");
         		// FINE MODIFICA
	...
	if (!empty ($fChange))
   	{
      		...
      		$result = db_query("SELECT * FROM $table_vacation WHERE email = '$fUsername'");
      		if($result['rows'] == 1) {
          		// Inserite datastart e datastop
          		// $result = db_query("UPDATE $table_vacation SET active = '$Active', subject = '$fSubject', body = '$fBody' created = NOW() WHERE email = '$fUsername'");
          		$result = db_query("UPDATE $table_vacation SET active = '$Active', subject = '$fSubject', body = '$fBody', datainizio = '$datastart', datafine = '$datastop' created = NOW() WHERE email = '$fUsername'");
      		}
      		else {
          		// Inserite datastart e datastop
          		...
          		$result = db_query ("INSERT INTO $table_vacation (email,subject,body,domain,created,active,datainizio,datafine) VALUES ('$fUsername','$fSubject','$fBody','$fDomain',NOW(),'$Active','$datastart','$datastop')");
      		}
